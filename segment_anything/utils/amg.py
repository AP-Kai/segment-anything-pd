    ...
    def filter(self, keep: paddle.Tensor) -> None:
        for k, v in self._stats.items():
            if v is None:
                ...
                self._stats[k] = v[paddle.to_tensor(keep)]
            elif isinstance(v, np.ndarray):
                self._stats[k] = v[keep.detach().cpu().numpy()]
            elif isinstance(v, list) and keep.dtype == paddle.bool:
    ...
    def cat(self, new_stats: "MaskData") -> None:
        for k, v in new_stats.items():
            if k not in self._stats or self._stats[k] is None:
                ...
                self._stats[k] = paddle.concat([self._stats[k], v], axis=0)
            elif isinstance(v, np.ndarray):
...
def batched_mask_to_box(masks: paddle.Tensor) -> paddle.Tensor:
    ...
    """
    # paddle.max below raises an error on empty inputs, just skip in this case
    if paddle.numel(masks) == 0:
        return paddle.zeros(*masks.shape[:-2], 4)
