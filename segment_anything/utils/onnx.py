...
class SamOnnxModel(nn.Layer):
    ...
    def resize_longest_image_size(
        ...
        input_image_size = input_image_size.to(paddle.float32)
        scale = longest_side / paddle.max(input_image_size)
        transformed_size = scale * input_image_size
        transformed_size = paddle.floor(transformed_size + 0.5).to(paddle.int64)
    ...
    def mask_postprocessing(self, masks: paddle.Tensor, orig_im_size: paddle.Tensor) -> paddle.Tensor:
        ...
        orig_im_size = orig_im_size.to(paddle.int64)
    ...
    def select_masks(
        ...
        score_reweight = paddle.tensor(
            [[1000] + [0] * (self.model.mask_decoder.num_mask_tokens - 1)]
        ).to(iou_preds.device)
        score = iou_preds + (num_points - 2.5) * score_reweight
        best_idx = paddle.argmax(score, dim=1)
