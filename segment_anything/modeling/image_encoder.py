...
class ImageEncoderViT(nn.Layer):
    def __init__(
            ...
            out_chans: int = 256,
            qkv_bias: bool = True,
            norm_layer: Type[paddle.nn.Layer] = nn.LayerNorm,
        ...
        if use_abs_pos:
            ...
            self.pos_embed = paddle.create_parameter(
                ...
                default_initializer=paddle.nn.initializer.Assign(
                    paddle.zeros(shape=(1, img_size // patch_size, img_size // patch_size, embed_dim))))
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        x = self.neck(paddle.transpose(x, [0, 3, 1, 2]))
...
class Block(nn.Layer):
    ...
    def __init__(
            ...
            mlp_ratio: float = 4.0,
            qkv_bias: bool = True,
            norm_layer: Type[nn.Layer] = paddle.nn.LayerNorm,
            act_layer: Type[nn.Layer] = nn.GELU,
            use_rel_pos: bool = False,
            rel_pos_zero_init: bool = True,
            window_size: int = 0,
...
class Attention(nn.Layer):
    ...
    def __init__(
        ...
        if self.use_rel_pos:
            ...
            self.rel_pos_h = paddle.create_parameter(shape=[2 * input_size[0] - 1, head_dim],
                                                     dtype='float32',
                                                     default_initializer=paddle.nn.initializer.Assign(paddle.zeros(
                                                         shape=(2 * input_size[0] - 1, head_dim)))
                                                     )
            self.rel_pos_w = paddle.create_parameter(shape=[2 * input_size[1] - 1, head_dim],
                                                     dtype='float32',
                                                     default_initializer=paddle.nn.initializer.Assign(paddle.zeros(
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        qkv = self.qkv(x)
        qkv = paddle.reshape(qkv, [B, H * W, 3, self.num_heads, -1])
        qkv = paddle.transpose(qkv, [2, 0, 3, 1, 4])
        # q, k, v with shape (B * nHead, H * W, C)
        q, k, v = paddle.reshape(qkv, [3, B * self.num_heads, H * W, -1]).unbind(0)

        attn = (q * self.scale) @ paddle.transpose(k, [0, 2, 1])
        ...
        x = (attn @ v)
        x = paddle.reshape(x, [B, self.num_heads, H, W, -1])
        x = paddle.transpose(x, [0, 2, 3, 1, 4])
...
def window_unpartition(
    ...
    x = paddle.reshape(windows, [B, Hp // window_size, Wp // window_size, window_size, window_size, -1])
    x = paddle.transpose(x, [0, 1, 3, 2, 4, 5])
    x = paddle.reshape(x, [B, Hp, Wp, -1])
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: paddle.Tensor) -> paddle.Tensor:
    ...
    if rel_pos.shape[0] != max_rel_dist:
        ...
        rel_pos_resized = F.interpolate(
            paddle.transpose(rel_pos.reshape(1, rel_pos.shape[0]), [0, 2, 1]),
            size=max_rel_dist,
            mode="linear",
        )
        rel_pos_resized = rel_pos_resized.reshape(-1, max_rel_dist)
        rel_pos_resized = paddle.transpose(rel_pos_resized, [1, 0])
    else:
...
def add_decomposed_rel_pos(
    ...
    B, _, dim = q.shape
    r_q = paddle.reshape(q, [B, q_h, q_w, dim])
    rel_h = paddle.einsum("bhwc,hkc->bhwk", r_q, Rh)
    rel_w = paddle.einsum("bhwc,wkc->bhwk", r_q, Rw)

    attn = (
            paddle.reshape(attn, [B, q_h, q_w, k_h, k_w])
            + rel_h[:, :, :, :, None] + rel_w[:, :, :, None, :]
    )
    attn = paddle.reshape(attn, [B, q_h * q_w, k_h * k_w])
    return attn
