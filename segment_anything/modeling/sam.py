...
class Sam(nn.Layer):
    ...
    def __init__(
        ...
        self.register_buffer("pixel_std", paddle.to_tensor(pixel_std).reshape([-1, 1, 1]), False)
    ...
    def forward(
        ...
        """
        input_images = paddle.stack([self.preprocess(x["image"]) for x in batched_input], axis=0)
    ...
    def preprocess(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        if x.ndim == 3:
            x = F.pad(x.unsqueeze(1), (0, padw, 0, padh), data_format="NCHW").squeeze(1)
        else:
            x = F.pad(x, (0, padw, 0, padh), data_format="NCHW")
        return x
