...
class PromptEncoder(nn.Layer):
    def __init__(
        ...
        mask_in_chans: int,
        activation: Type[nn.Layer] = nn.GELU,
    ) -> None:
        """
        ...
        self.point_embeddings = nn.LayerList(point_embeddings)
    ...
    def _embed_points(
        ...
        if pad:
            padding_point = paddle.zeros((points.shape[0], 1, 2), dtype=points.dtype)
            padding_label = -paddle.ones((labels.shape[0], 1), dtype=labels.dtype)
            points = paddle.concat([points, padding_point], axis=1)
            labels = paddle.concat([labels, padding_label], axis=1)
        point_embedding = self.pe_layer.forward_with_coords(points, self.input_image_size)
        if (labels == -1).any():
            point_embedding[labels == -1] = paddle.zeros((1,), dtype=point_embedding.dtype)
    ...
    def _get_device(self) -> paddle.device:
        return self.point_embeddings[0].weight
    ...
    def forward(
        ...
        sparse_embeddings = paddle.empty((bs, 0, self.embed_dim))
        if points is not None:
            coords, labels = points
            point_embeddings = self._embed_points(coords, labels, pad=(boxes is None))
            sparse_embeddings = paddle.concat([sparse_embeddings, point_embeddings], axis=1)
        if boxes is not None:
            box_embeddings = self._embed_boxes(boxes)
            sparse_embeddings = paddle.concat([sparse_embeddings, box_embeddings], axis=1)
...
class PositionEmbeddingRandom(nn.Layer):
    ...
    def __init__(self, num_pos_feats: int = 64, scale: Optional[float] = None) -> None:
        ...
        self.register_buffer(
            ...
            scale * paddle.randn((2, num_pos_feats)),
        )
    ...
    def _pe_encoding(self, coords: paddle.Tensor) -> paddle.Tensor:
        ...
        return paddle.concat([paddle.sin(coords), paddle.cos(coords)], axis=-1)

    def forward(self, size: Tuple[int, int]) -> paddle.Tensor:
        """Generate positional encoding for a grid of the specified size."""
        h, w = size
        grid = paddle.ones((h, w), dtype=paddle.float32)
        y_embed = grid.cumsum(axis=0) - 0.5
        x_embed = grid.cumsum(axis=1) - 0.5
        y_embed = y_embed / h
        x_embed = x_embed / w

        pe = self._pe_encoding(paddle.stack([x_embed, y_embed], axis=-1))
        return pe.transpose([2, 0, 1])  # C x H x W

    def forward_with_coords(
        self, coords_input: paddle.Tensor, image_size: Tuple[int, int]
    ) -> paddle.Tensor:
