...
class MaskDecoder(nn.Layer):
    def __init__(
        ...
        *,
        ...
        self.output_hypernetworks_mlps = nn.LayerList(
            [
    ...
    def predict_masks(
        ...
        output_tokens = paddle.concat([self.iou_token.weight, self.mask_tokens.weight], axis=0)
        output_tokens = output_tokens.unsqueeze(0).expand(
            [sparse_prompt_embeddings.shape[0], -1, -1]
        )
        tokens = paddle.concat((output_tokens, sparse_prompt_embeddings), axis=1)
        ...
        hyper_in = paddle.stack(hyper_in_list, axis=1)
